  ...
  return (
    <main className="overflow-hidden bg-[#f4f8f8] text-[#153640] dark:bg-background dark:text-foreground">
      ...
      <section hidden={!pathway.isVisible} id="urban-care-pathway" className="relative overflow-hidden bg-[#e0f3f0] py-20 dark:bg-[#112d32] lg:py-28">
        <div className="container-pstc"><div className="mx-auto max-w-3xl text-center"><p className="text-xs font-black uppercase tracking-[.28em] text-[#168f81] dark:text-[#5ed7c7]">{pathway.eyebrow}</p><h2 className="mt-4 text-4xl font-black leading-tight tracking-[-.045em] sm:text-5xl">{pathway.title}<span className="block text-[#e86145]">{pathway.highlightedTitle}</span></h2><p className="mx-auto mt-6 max-w-2xl text-base leading-8 text-muted-foreground">{pathway.description}</p></div>
          <div className="relative mt-14 grid gap-4 lg:grid-cols-5"><div className="absolute left-[7%] right-[7%] top-8 hidden h-1 bg-gradient-to-r from-[#20bea9] via-[#4d8fd6] to-[#e86145] lg:block" />{(pathway.items ?? []).map((item: Record<string, string>, index: number) => <article key={`${item.step}-${index}`} className={`relative rounded-[1.5rem] border border-white/60 bg-background p-5 shadow-[0_14px_40px_rgba(12,57,66,.07)] ${index % 2 ? "lg:mt-10" : ""}`}><span className="relative grid size-14 place-items-center rounded-full border-4 border-background bg-[#0c3942] font-mono text-xs font-black text-[#62e2d1]">{item.step}</span><p className="mt-6 text-[10px] font-black uppercase tracking-[.2em] text-[#e86145]">{item.verb}</p><h3 className="mt-2 text-lg font-black">{item.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{item.description}</p></article>)}</div>
        </div>
      </section>
      ...
      <section hidden={!components.isVisible} className="py-20 lg:py-28">
        <div className="container-pstc"><div className="grid gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-end lg:gap-20"><div><p className="text-xs font-black uppercase tracking-[.28em] text-[#168f81] dark:text-[#5ed7c7]">{components.eyebrow}</p><h2 className="mt-4 text-4xl font-black leading-tight tracking-[-.045em] sm:text-5xl">{components.title}<span className="block text-[#168f81] dark:text-[#5ed7c7]">{components.highlightedTitle}</span></h2></div><p className="max-w-2xl text-base leading-8 text-muted-foreground lg:justify-self-end">{components.description}</p></div>
          <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{(components.items ?? []).map((item: Record<string, string>, index: number) => { const Icon = componentIconMap[item.icon] ?? Stethoscope; return <article key={`${item.number}-${index}`} className="group rounded-[1.7rem] border border-border bg-card p-7 transition hover:-translate-y-1 hover:shadow-[0_20px_55px_rgba(12,57,66,.09)]"><div className="flex items-center justify-between"><span className={`grid size-12 place-items-center rounded-2xl ${componentTones[index % componentTones.length]}`}><Icon className="size-5" /></span><span className="font-mono text-xs font-black text-[#168f81]/40">{item.number}</span></div><h3 className="mt-7 text-xl font-black">{item.title}</h3><p className="mt-3 text-sm leading-7 text-muted-foreground">{item.description}</p></article>; })}</div>
        </div>
      </section>

      <section hidden={!access.isVisible} className="bg-[#0c3942] py-20 text-white lg:py-28"><div className="container-pstc"><div className="max-w-3xl"><p className="text-xs font-black uppercase tracking-[.28em] text-[#62e2d1]">{access.eyebrow}</p><h2 className="mt-4 text-4xl font-black leading-tight tracking-[-.045em] sm:text-5xl">{access.title}<span className="block text-[#ff987f]">{access.highlightedTitle}</span></h2><p className="mt-6 max-w-2xl text-base leading-8 text-white/55">{access.description}</p></div><div className="mt-12 grid gap-px overflow-hidden rounded-[2rem] bg-white/15 md:grid-cols-2 lg:grid-cols-4">{(access.items ?? []).map((item: Record<string, string>, index: number) => <article key={`${item.title}-${index}`} className="bg-[#0e424c] p-7"><span className="text-[10px] font-black uppercase tracking-[.2em] text-[#62e2d1]">{item.keyword}</span><h3 className="mt-8 text-2xl font-black">{item.title}</h3><p className="mt-4 text-sm leading-7 text-white/50">{item.description}</p></article>)}</div></div></section>
