  ...
  return (
    <main className="overflow-hidden bg-[#f6f1e9] text-[#251c3d] dark:bg-background dark:text-foreground">
      ...
      <section hidden={!capacities.isVisible} id="focus-capacity" className="bg-[#eae4f8] py-20 dark:bg-[#1d1830] lg:py-28">
        <div className="container-pstc"><div className="grid gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-end lg:gap-20"><div><p className="text-xs font-black uppercase tracking-[.28em] text-[#7754cf] dark:text-[#aa94ed]">{capacities.eyebrow}</p><h2 className="mt-4 text-4xl font-black leading-tight tracking-[-.045em] sm:text-5xl">{capacities.title}<span className="block text-[#d73d61] dark:text-[#ff8099]">{capacities.highlightedTitle}</span></h2></div><p className="max-w-2xl text-base leading-8 text-muted-foreground lg:justify-self-end">{capacities.description}</p></div>
          <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{(capacities.items ?? []).map((item: Record<string, string>, index: number) => { const Icon = capacityIconMap[item.icon] ?? Compass; return <article key={`${item.number}-${index}`} className="group relative overflow-hidden rounded-[1.7rem] border border-white/60 bg-background p-7 shadow-[0_15px_45px_rgba(33,24,59,.07)] transition hover:-translate-y-1"><span className={`grid size-12 place-items-center rounded-2xl ${capacityTones[index % capacityTones.length]}`}><Icon className="size-5" /></span><span className="absolute right-4 top-1 text-7xl font-black text-[#7754cf]/[.06]">{item.number}</span><h3 className="mt-7 text-xl font-black">{item.title}</h3><p className="mt-3 text-sm leading-7 text-muted-foreground">{item.description}</p></article>; })}</div>
        </div>
      </section>

      <section hidden={!pathway.isVisible} className="relative overflow-hidden bg-[#21183b] py-20 text-white lg:py-28"><div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(255,92,124,.15),transparent_27%),radial-gradient(circle_at_15%_85%,rgba(84,212,208,.12),transparent_25%)]" /><div className="container-pstc relative"><div className="mx-auto max-w-3xl text-center"><p className="text-xs font-black uppercase tracking-[.28em] text-[#54d4d0]">{pathway.eyebrow}</p><h2 className="mt-4 text-4xl font-black leading-tight tracking-[-.045em] sm:text-5xl">{pathway.title}<span className="block text-[#ffcc48]">{pathway.highlightedTitle}</span></h2><p className="mx-auto mt-6 max-w-2xl text-base leading-8 text-white/52">{pathway.description}</p></div><div className="relative mt-14 grid gap-4 lg:grid-cols-5"><div className="absolute left-[7%] right-[7%] top-7 hidden border-t-2 border-dashed border-white/20 lg:block" />{(pathway.items ?? []).map((item: Record<string, string>, index: number) => <article key={`${item.step}-${index}`} className={`relative rounded-[1.5rem] border border-white/10 bg-white/[.06] p-5 backdrop-blur ${index % 2 ? "lg:mt-10" : ""}`}><span className={`relative grid size-14 place-items-center rounded-full border-4 border-[#21183b] font-mono text-xs font-black ${capacityTones[index % capacityTones.length]}`}>{item.step}</span><p className="mt-6 text-[10px] font-black uppercase tracking-[.2em] text-[#54d4d0]">{item.verb}</p><h3 className="mt-2 text-lg font-black">{item.title}</h3><p className="mt-2 text-sm leading-6 text-white/45">{item.description}</p></article>)}</div></div></section>
